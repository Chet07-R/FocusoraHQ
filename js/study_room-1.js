...
function editTask(id) {
  ...
  if (newText !== null && newText.trim() !== "") {
    ...
    renderTasks();
    updateTaskCount();
    showNotification('✏️', 'Edited', 'Task updated');
  }
}
  ...
  list.innerHTML = filteredTasks.map(task => {
    return `
          <li class="bg-white/10 backdrop-blur-sm border border-white/20 p-3 rounded-lg flex items-start gap-3 group hover:bg-white/15 transition-all ${task.completed ? 'opacity-60' : ''}">
            ...
            <div class="flex-1">
              <span ondblclick="editTask(${task.id})" class="${task.completed ? 'line-through text-white/50' : 'text-white cursor-text'}">
                ${task.text}
              </span>
            </div>
...
function exportNotes(type) {
  ...
  a.href = url;
  a.download = `notes-${new Date().toISOString().split('T')[0]}.${ext}`;
  a.click();
...
function startDictation() {
  ...
  if (!SpeechRecognition) {
    showNotification('⚠️', 'Not Supported', 'Speech recognition not available in this browser.');
    return;
  }
  ...
  voiceBtn.classList.add('bg-emerald-500/40');

  recognition.onresult = (event) => {
    const transcript = event.results[event.results.length - 1][0].transcript.trim();
    const existing = area.innerText.trim();
    area.innerText = (existing + ' ' + transcript).trim();
    updateCharCount();
    saveNotes();
  };
  ...
  recognition.onerror = (event) => {
    console.error(event);
    showNotification('❌', 'Speech Error', event.error || 'Recognition failed');
  };

  recognition.onend = () => {
    voiceBtn.textContent = '🎤 Speak';
    voiceBtn.classList.remove('bg-emerald-500/40');
    showNotification('✅', 'Done', 'Speech added to notes');
  };

  recognition.start();
  showNotification('🎤', 'Listening', 'Start speaking...');
}
...
function loadVoices() {
  ...
  selectedVoice =
    ...
    voices.find(v => /female/i.test(v.name) && /english/i.test(v.lang)) ||
...
function readNotesAloud() {
  ...
  if (isSpeaking) {
    ...
    voiceBtn.classList.remove('bg-indigo-500/40');
    showNotification('⏹️', 'Stopped', 'Speech stopped');
    return;
  }

  // Nothing to read
  if (!text) {
    showNotification('⚠️', 'Empty Notes', 'There is nothing to read aloud.');
    return;
  }

  // Browser check
  if (!('speechSynthesis' in window)) {
    showNotification('⚠️', 'Not Supported', 'Text-to-speech not supported');
    return;
  }

  // Load voices if not already
  ...
  voiceBtn.classList.add('bg-indigo-500/40');
  isSpeaking = true;
  ...
  utterance.onend = () => {
    ...
    voiceBtn.textContent = '🔊 Listen';
    voiceBtn.classList.remove('bg-indigo-500/40');
    showNotification('✅', 'Done', 'Finished reading your notes');
  };

  window.speechSynthesis.speak(utterance);
  showNotification('🔊', 'Reading', `Using ${selectedVoice?.name || 'default'} voice`);
}
...
function formatText(format) {
  ...
  // Update button states after format toggle
  updateFormatButtons();
  updateCharCount();
}
...
function updateFormatButtons() {
  const buttons = {
    ...
    underline: document.querySelector('[data-format="underline"]'),
    list: document.querySelector('[data-format="list"]'),
  };

  if (!buttons.bold) return;
  ...
  buttons.bold.classList.toggle('active-format', isBold);
  buttons.italic.classList.toggle('active-format', isItalic);
  buttons.underline.classList.toggle('active-format', isUnderline);
  buttons.list.classList.toggle('active-format', isList);
}

// Keep button states updated when user clicks inside the notes area
document.addEventListener('selectionchange', () => {
  const area = document.getElementById('notesArea');
  if (area && area.contains(window.getSelection().anchorNode)) {
    updateFormatButtons();
  }
});
document.addEventListener('keydown', (e) => {
  if (!document.getElementById('notesArea').contains(document.activeElement)) return;
...
function insertBullet() {
  ...
  document.execCommand('insertUnorderedList'); // works again once bullets are visible
  updateCharCount();
}
